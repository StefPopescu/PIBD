...
public class JavaBean {
	...
	public void connect() throws ClassNotFoundException, SQLException, Exception {
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/tema?useSSL=false", "root", "lunabetiluna");
	...
	public void connect(String bd) throws ClassNotFoundException, SQLException, Exception {
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + bd, "root", "lunabetiluna");
		} catch (ClassNotFoundException cnfe) {
			error = "ClassNotFoundException: Nu s-a gasit driverul bazei de date.";
			throw new ClassNotFoundException(error);
		} catch (SQLException cnfe) {
	...
	public void connect(String bd, String ip) throws ClassNotFoundException, SQLException, Exception {
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			con = DriverManager.getConnection("jdbc:mysql://" + ip + ":3306/" + bd, "root", "lunabetiluna");
		} catch (ClassNotFoundException cnfe) {
			error = "ClassNotFoundException: Nu s-a gasit driverul bazei de date.";
			throw new ClassNotFoundException(error);
		} catch (SQLException cnfe) {
	...
	public void adaugaOras(String Nume)
		...
		if (con != null) {
			try {
				...
				stmt.executeUpdate("insert into orase(Nume) values('" + Nume + "');");
	...
	public void adaugaTeatru(int idoras, String Nume, int Capacitate, String Adresa)
			throws SQLException, Exception {
		if (con != null) {
			try {
				// create a prepared SQL statement
				Statement stmt;
				stmt = con.createStatement();
				stmt.executeUpdate("insert into teatre(idoras, Nume, Capacitate, Adresa) values('" + idoras + "' , '" + Nume + "'  , '" + Capacitate + "', '" + Adresa + "');");
	...
	public void adaugaActor(int idteatru, String Nume, String Prenume, int Salariu)
			throws SQLException, Exception {
		if (con != null) {
			try {
				// create a prepared SQL statement
				Statement stmt;
				stmt = con.createStatement();
				stmt.executeUpdate("insert into actori(idteatru, Nume, Prenume, Salariu) values('" + idteatru + "'  , '" + Nume + "', '" + Prenume + "', '" + Salariu + "');");
	...
	public ResultSet vedeOrase() throws SQLException, Exception {
		...
		try {
			String queryString = ("SELECT \r\n"
					+ "    orase.idoras,\r\n"
					+ "    orase.nume NumeOras,\r\n"
					+ "    teatre.idteatru IDteatru,\r\n"
					+ "    teatre.nume NumeTeatru,\r\n"
					+ "    actori.idactor IDactor,\r\n"
					+ "    actori.nume NumeActor,\r\n"
					+ "    actori.prenume\r\n"
					+ "FROM\r\n"
					+ "    orase\r\n"
					+ "        LEFT JOIN\r\n"
					+ "    teatre ON orase.idoras = teatre.idoras\r\n"
					+ "        LEFT JOIN\r\n"
					+ "    actori ON teatre.idteatru = actori.idteatru;\r\n"
					);
			Statement stmt = con.createStatement(/*ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY*/);
			rs = stmt.executeQuery(queryString);
		} catch (SQLException sqle) {
